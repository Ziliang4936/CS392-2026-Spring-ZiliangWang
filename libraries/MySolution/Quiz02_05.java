...
public class Quiz02_05 {
    ...
    //   An RBT whose levels strictly alternate black, red,
    //   black, red, ..., starting from a black root, can
    //   hold at most 4^b - 1 nodes for black-height b.
    //   Therefore the minimum achievable black-height for
    //   N nodes is ceil(log_4(N + 1)).
    //   With N = 1_000_000: 4^10 - 1 = 1_048_575 >= N,
    //   so b_min = 10.
    //
    //   genRedBLackBST builds a heap-indexed complete binary
    //   tree of N nodes; the colour at depth d is BLACK when
    //   d is even and RED when d is odd, giving 10 black
    //   layers (depths 0,2,...,18). Keys 0..999999 are
    //   assigned by in-order traversal so the tree is a BST.
    //   Every root-to-NIL path crosses the same 10 black
    //   nodes, confirming black-height = 10 = b_min.
    //
