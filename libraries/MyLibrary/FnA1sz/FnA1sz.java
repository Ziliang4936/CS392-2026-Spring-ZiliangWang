...
//
	...
	return FnA1szSUtil.rfolditm(this, FnListSUtil.<T>nil(),
	    (x, acc) -> FnListSUtil.cons(x, acc));
    }
    public FnList<T> rlistize() {
	return FnA1szSUtil.folditm(this, FnListSUtil.<T>nil(),
	    (acc, x) -> FnListSUtil.cons(x, acc));
    }
//
    public FnA1sz<T>
	mergeSort(ToIntBiFunction<T,T> cmp) {
	return new FnA1sz<T>(FnListSUtil.mergeSort(this.listize(), cmp));
    }
//
    public FnA1sz<T>
	insertSort(ToIntBiFunction<T,T> cmp) {
	return new FnA1sz<T>(FnListSUtil.insertSort(this.listize(), cmp));
    }
//
